...
export default function DirectionsSection({register, control, errors, directionFields, addInputRows, removeInputRows, addOrUpdate}) {
  ...
  return (
      <div className="formSection">
        ...
        <h5 style={{marginBottom: '1em'}}><b>DIRECTIONS</b>   <button type="button" className="addInputRows" onClick={() => addInputRows("direction")}>+</button></h5>
        {directionFields.map((direction, index) => {
        return (<Row key={`directions[${index}].instructions`} className="mb-3 directionRow">
